"""
Sortie 1e: Quarterly TSR-1G Structural Panel -- Tunisia 2008 Q1 through 2011 Q1.
Corrected quarterly analysis applying audit fix D.4 to sortie 1d.

Change vs sortie 1d (D.4 -- repression double-counting):
  Repression was entering T_tension_q (weight 0.20) AND sigma_valve (weight 0.50)
  AND R_repression simultaneously. Section 35.2 routes R -> T at t+1, not as a
  same-period input. Removed from T_tension_q; weights redistributed:
    1d: youth_n*0.35 + gini_n*0.30 + rep_n*0.20 + protest_n*0.15
    1e: youth_n*0.45 + gini_n*0.40 + protest_n*0.15
  Repression routes only through sigma_valve and the discharge formula.

All other 1d mechanics unchanged:
  - FFPI at quarterly resolution (FAO monthly archive)
  - Absolute FH scale for civil liberties / political rights
...
# D.4 fix: repression removed. Theory routes R->T at t+1 (Section 35.2).
# Repression routes through sigma_valve and discharge only.
df['T_tension_q'] = (
      youth_n    * 0.45
    + gini_n     * 0.40
    + protest_n  * 0.15
